...
Util.buildItemDetail = async function(data) {
  let display
  if (data) {
    display = '<div id="item-detail-display">'
    // Usamos la imagen grande (inv_image) como pide la tarea
    display += `<img src="${data.inv_image}" alt="Image of ${data.inv_make} ${data.inv_model} on CSE Motors">`
    display += '<div class="detail-info">'
    display += `<h2>${data.inv_make} ${data.inv_model} Details</h2>`
    
    // Precio formateado como U.S. Dollars
    ...
    display += `<p><strong>Miles:</strong> ${mileage}</p>`
    display += '</div>'
    display += '</div>'
  } else {
...
Util.checkAccountType = (req, res, next) => {
  ...
  if (res.locals.loggedin) {
    ...
    // 2. ¿Es empleado o administrador?
    if (account_type === "Employee" || account_type === "Admin") {
      next() // ¡Adelante, pase usted!
    } else {
      // Es un cliente intentando hackear la URL
